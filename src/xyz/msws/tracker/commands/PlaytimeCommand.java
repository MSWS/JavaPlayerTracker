...
public class PlaytimeCommand extends AbstractCommand {
    ...
    public void execute(Message message, String[] args) {
        ...
        duration += " to " + (setTo ? TimeParser.getDateDescription(to) : "now");
    ...
    private List<Entry<ServerPlayer, Long>> getRankings(long start, long end, String server,
        ...
        List<Entry<ServerPlayer, Long>> ranked = new ArrayList<>(getPlaytimes(start, end, server).entrySet());
        ranked.sort(comp);
        return ranked;
    }

    private List<Entry<ServerPlayer, Long>> getRankings(long start, long end, String server) {
        return getRankings(start, end, server, (o1, o2) -> o2.getValue().compareTo(o1.getValue()));
    }
